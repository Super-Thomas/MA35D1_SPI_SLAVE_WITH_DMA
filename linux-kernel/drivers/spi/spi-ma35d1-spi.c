...
#define SLAVE_MODE      (0x01 << 18)  /* Added for SPI slave mode control in SPI_CTL register, bit 18 */
...
#define SPI_GENERAL_TIMEOUT_MS	60000
...
static void nuvoton_spi_hw_init(struct nuvoton_spi *hw)
{
	...
	if (hw->master->slave) {
		__raw_writel(__raw_readl(hw->regs + REG_CTL) | SLAVE_MODE, hw->regs + REG_CTL);
	} else {
		__raw_writel(__raw_readl(hw->regs + REG_CTL) & ~SLAVE_MODE, hw->regs + REG_CTL);
	}
...
static void nuvoton_spi_set_cs(struct spi_device *spi, bool lvl)
{
	...
	if (spi->master->slave)
		return; // In slave mode, CS is controlled by external master, driver doesn't need to set CS level.
